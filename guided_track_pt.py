...
# --- MOVED --- Function moved up to be available for ExemplarEmbedder
...
class ExemplarEmbedder:
    ...
    # --- MODIFIED --- Added segment_exemplars and save_segmented_path arguments
    @torch.no_grad()
    def embed_exemplar_dir(self, d, mode="mean", segment_exemplars=False, save_segmented_path=None):
        ...
        for p in paths[:8]:  # up to 8 exemplars
            try:
                ...
                # --- NEW --- Apply GrabCut segmentation to exemplar
                bgr_img = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)
                h, w = bgr_img.shape[:2]

                # Use a 5% inset for the GrabCut rect, assuming object is centered
                rx1 = int(w * 0.05);
                ry1 = int(h * 0.05)
                rx2 = int(w * 0.95);
                ry2 = int(h * 0.95)
                ...
                # Use more iterations for (hopefully) better quality on exemplars
                ...
                mask_b = (mask > 0)
                masked_rgb[~mask_b] = 0  # Black out background

                final_pil_img = Image.fromarray(masked_rgb)
                imgs.append(final_pil_img)
                # --- End segmentation ---

                # --- NEW --- Save the segmented exemplar if path is provided
...
def main():
    ...
    # --- NEW --- Argument to segment exemplars
    ap.add_argument("--seg_exemplars", action="store_true", help="Apply GrabCut segmentation to exemplar images")
    ...
    # --- MODIFIED --- Create save path for segmented exemplars and pass flag to embedder
